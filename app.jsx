    ...
    <div style={{ maxWidth:720, display:'flex', flexDirection:'column', gap:16 }}>
      ...
      <div style={{ background:'var(--panel)', border:'1px solid var(--border)', borderRadius:14, padding:22 }}>
        ...
        <div onClick={() => set('crm_lookalike_auto', !cfg.crm_lookalike_auto)}
          ...
          <div>
            <div style={{ fontSize:13.5, fontWeight:500 }}>
              {cfg.crm_lookalike_auto ? 'Aprendizado automático ligado' : 'Aprendizado automático desativado'}
            </div>
            <div style={{ fontSize:12, color:'var(--faint)', marginTop:2 }}>
              {cfg.crm_lookalike_auto
                ? 'Cada lista que o CRM alimenta (ex.: "Semelhantes automático") tem um radar de semelhantes: a cada compra ele refaz o perfil e volta a buscar. Liga a partir de 3 clientes.'
                : 'As conversões são guardadas na lista, mas não geram radar automático. A lista ainda pode ser usada num radar ou numa pauta da Estratégia.'}
            </div>
          </div>
        </div>
...
function LeadDetailPanel({ leadId, onClose, onCrm, onStatusChange }) {
  ...
  const regerarSwot = async () => {
    ...
    try {
      ...
      window.alert('A análise ainda não voltou depois de quase 3 minutos. Ela pode chegar mesmo assim — ' +
        'reabra o lead daqui a pouco antes de tentar de novo. Se continuar sem vir, o motivo costuma ser o modelo ' +
        'configurado em Integrações → Inteligência: modelos de raciocínio gastam o orçamento de tokens pensando e ' +
        'não sobra nada pra resposta. Um modelo direto (ex.: openai/gpt-4o-mini) resolve.');
    } finally { setRegSwot(false); }
  };
